...
-(id)initWithFrameRect:(CGRect)rect ImageArray:(NSArray *)imgArr TitleArray:(NSArray *)titArr
{
	...
	if ((self=[super initWithFrame:rect])) {
        ...
        for (int i=0; i< pageCount; i++)
        {
            ...
            {
                // 网络图片 请使用ego异步图片库
                [tools imageView:imgView setImageWithURL:[NSURL URLWithString:imgURL] indicatorStyle:UIActivityIndicatorViewStyleGray];
            }
            else
            {
...
//        pageControl=[[UIPageControl alloc]initWithFrame:CGRectMake((self.frame.size.width-pageControlWidth),6, pageControlWidth, pagecontrolHeight)];
        ...
        noteTitle=[[UILabel alloc] initWithFrame:CGRectMake(0, 6, self.frame.size.width-pageControlWidth, pagecontrolHeight)];
        [noteTitle setTextColor:[UIColor whiteColor]];
        [noteTitle setText:[@"  " stringByAppendingString:[titleArray objectAtIndex:0]]];
...
- (void)scrollViewDidScroll:(UIScrollView *)sender
{
    ...
    [noteTitle setText:[@"  " stringByAppendingString:[titleArray objectAtIndex:titleIndex]]];
}
...
-(void)gotoNextPage
{
    ...
    currentPage += 1;
    if(currentPage == [imageArray count] - 2)
        pageControl.currentPage = 0;
    else
        pageControl.currentPage = currentPage;
    currentPageIndex = pageControl.currentPage + 1;
    [scrollView scrollRectToVisible:CGRectMake(self.frame.size.width * currentPageIndex,0,self.frame.size.width,self.frame.size.height) animated:YES]; // 最后+1,循环第1页
}
